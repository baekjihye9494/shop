...
public class GoodsService {
	...
	//고객 상품 리스트 ㅔ이지 요청
	public List<Map<String,Object>> getcustomerGoodsListByPage(Connection conn, int rowPerPage, int beginRow, Goods goods) throws Exception {
		...
		try {
			...
			list = goodsDao.updateGoods(conn, goods);
			int row = 0;
			if(row != 0) {
				list = goodsImgDao.updateGoodsImg(conn, goodsImg);
				if(row == 0) {
